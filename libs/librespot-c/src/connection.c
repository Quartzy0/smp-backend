...
static uint8_t sp_aes_iv[] = {0x72, 0xe0, 0x67, 0xfb, 0xdd, 0xcb, 0xcf, 0x77, 0xeb, 0xe8, 0xbc, 0x64, 0x3f, 0x63, 0x0d,
                              0x93};
...
response_client_hello(uint8_t *msg, size_t msg_len, struct sp_session *session) {
    ...
    crypto_shared_secret(
            ...
            apresponse->challenge->login_crypto_challenge->diffie_hellman->gs.data,
            apresponse->challenge->login_crypto_challenge->diffie_hellman->gs.len);
...
response_aes_key(uint8_t *payload, size_t payload_len, struct sp_session *session) {
    ...
    ret = crypto_aes_new(&channel->file.decrypt, channel->file.key, sizeof(channel->file.key), sp_aes_iv,
                         sizeof(sp_aes_iv), &errmsg);
    if (ret < 0)
